const installIubenda = function (lang, iubendaShouldLog = true, $store = undefined, baseUrl = 'https://www.myarstudio.cloud') {
    ...
    if ($store) {
        $store.dispatch('iubenda/setShouldLog', iubendaShouldLog);
    }
    ...
    if (process === undefined || (process.client !== undefined && process.client)) {
        ...
        iubendaShouldLog &&
            console.log('App.vue installIubenda language: ', lang, 'siteId:', siteId, 'cookiePolicyId:', cookiePolicyId);
        ...
        window._iub.csConfiguration = {
            ...
            'callback': {
                'onPreferenceExpressedOrNotNeeded': (preference) => {
                    ...
                    if ($store) {
                        $store.dispatch('iubenda/setCcpaOptedOut', ccpaOptedOut);
                    }
                    ...
                    if (!preference) {
                        ...
                        if ($store) {
                            $store.dispatch('iubenda/setPreferenceNotNeeded', true);
                        }
                        iubendaShouldLog && console.log('Iubenda cb: GTag dataLayer push ', 'iubenda_preference_not_needed');
                    }
                    else {
                        if ($store) {
                            $store.dispatch('iubenda/setPreferenceNotNeeded', false);
                        }
                        if (preference.consent === true) {
                            ...
                            if ($store) {
                                $store.dispatch('iubenda/setConsetGiven', true);
                            }
                            ...
                            if ($store) {
                                $store.dispatch('iubenda/setConsetRejected', true);
                            }
                            iubendaShouldLog && console.log('Iubenda cb: GTag dataLayer push ', 'iubenda_consent_rejected');
                        }
                        else if (preference.purposes) {
                            if ($store) {
                                $store.dispatch('iubenda/resetConsetGivenPurpose');
                            }
                            for (const purposeId in preference.purposes) {
                                if (preference.purposes[purposeId]) {
                                    ...
                                    if ($store) {
                                        $store.dispatch('iubenda/setConsetGivenPurpose', { iPurpose: purposeId, bValue: true });
                                    }
